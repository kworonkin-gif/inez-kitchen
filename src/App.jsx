...
export default function MealPlanner() {
  const [apiKey, setApiKey] = useState(() => {
    try { return localStorage.getItem("inez_groq_key") || ""; } catch { return ""; }
  });
      ...
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#fef9f0,#fde8d0 50%,#fef0e8)", fontFamily: "Georgia,serif", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ background: "white", borderRadius: 20, padding: 32, maxWidth: 400, width: "100%", boxShadow: "0 4px 24px rgba(0,0,0,0.1)" }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            />
            <p style={{ fontSize: 11, color: "#aaa", lineHeight: 1.6 }}>Get your free key at <strong>console.groq.com</strong> → API Keys → Create API key. Your key is stored only on this device.</p>
          </div>
          <button
            onClick={() => {
              const key = apiKeyInput.trim();
              if (key.length > 20) {
                localStorage.setItem("inez_groq_key", key);
                setApiKey(key);
              } else {
...
[snack] — [why good + how to serve]`;
    ...
    } catch (e) { setWeekPlan("❌ Error: " + e.message + ". Tap Change Key (top right) if your key looks wrong."); }
